...
@Component({
  ...
  template: `
    <ng-container *ngFor="let symbol of (settings?.symbol || []); index as index; first as first; last as last">
      ...
      <h2 *ngIf="settings?.symbol.length > 1">
      ...
      <div class="symbol-list">
        <div *ngIf="allowNullSelection"
             class="symbol-thumb"
             [class.active]="symbol.content === null"
             (click)="onSymbolSelected(symbol, null)">
        ...
        <div *ngIf="symbolSvc.allowSymbolUpload" class="symbol-thumb">
          <label style="display: block; cursor: pointer;">
            ...
            <input type="file" accept="image/*" (change)="fileChanged(symbol, $event)" style="display:none"/>
          </label>
        </div>

        <div *ngFor="let availableSymbol of symbolSvc.available"
             class="symbol-thumb"
             [class.active]="availableSymbol == symbol.content"
             (click)="onSymbolSelected(symbol, availableSymbol)">
          <img [src]="symbol | fn:getPreview:this:availableSymbol"/>
        </div>
      </div>
      ...
      <div class="flex">
        <label>
          ...
          <mat-slider [(ngModel)]="symbol.x" (input)="symbol.x = $event.value; onChange()"
                      [disabled]="symbol.content === null" [min]="0" [max]="100" [thumbLabel]="true"></mat-slider>
        </label>
        <label>
          <ng-container i18n>Posición vertical</ng-container>
          <mat-slider [(ngModel)]="symbol.y" (input)="symbol.y = $event.value; onChange()"
                      [disabled]="symbol.content === null" [min]="0" [max]="100" [thumbLabel]="true"></mat-slider>
        </label>
      </div>
      ...
      <div class="flex">
        <label>
          ...
          <mat-slider [(ngModel)]="symbol.size" (input)="symbol.size = $event.value; onChange()"
                      [disabled]="symbol.content === null" [min]="1" [max]="200" [thumbLabel]="true"></mat-slider>
        </label>
        <label>
          <ng-container i18n>Rotación</ng-container>
          <mat-slider [(ngModel)]="symbol.rotation" (input)="symbol.rotation = $event.value; onChange()"
                      [disabled]="symbol.content === null" [min]="-180" [max]="180" [step]="5" [thumbLabel]="true"></mat-slider>
        </label>
      </div>

      <div>
        <mat-slide-toggle [(ngModel)]="symbol.trim" (ngModelChange)="onChange()"
                          [disabled]="symbol.content === null" i18n>Recortar
        </mat-slide-toggle>
      </div>

      <div *ngIf="symbolSvc.isConfigurable(symbol)" style="margin-top: 10px">
        <label>
          <ng-container i18n>Color</ng-container>
          <color-picker [(ngModel)]="symbol.color" (ngModelChange)="onChange()"></color-picker>
        </label>
      </div>

      <div style="text-align: center; margin:5px 0">
        <button *ngIf="symbol.content && last" mat-raised-button (click)="addSymbol()">
