import { Component, DOCUMENT, effect, inject, model, untracked } from '@angular/core';
      ...
      @if (value()?.text.length > 1) {
        <mat-form-field appearance="fill" class="no-label-select">
          <mat-select [(ngModel)]="selectedText" (ngModelChange)="onChange()">
            @for (text of value().text || []; track text) {
...
export class NgShieldSettingsTextComponent implements FormValueControl<NgShieldSettings | null> {
  // Deps
  ...
  protected addText() {
    ...
    newText.y = Math.min(90, newText.y + this.value().text.length * 10);
    this.value().text.push(newText);
    this.selectedText = newText;
    this.onChange();
  }

  protected deleteText(text: NgShieldSettingsText) {
    const index = this.value().text.indexOf(text);
    if (index > -1 && text == this.selectedText) {
      this.value().text.splice(index, 1);
      this.selectedText = this.value().text[Math.max(index - 1, 0)];
    }
